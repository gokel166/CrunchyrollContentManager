...
func setupLog() {
	structlog.DefaultLogger.AppendPrefixKeys(LogRemoteIP, LogGRPCCode, LogFunc).
		SetSuffixKeys(
			LogServer, LogUserName, "dump", structlog.KeyStack,
		).
		SetDefaultKeyvals(structlog.KeyPID, nil).
